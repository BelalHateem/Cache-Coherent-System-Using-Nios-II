-SOFTWARE VERSION OF MATRIX MULTIPLICATION SYSTEM UTILIZING 2 CORES:
...
->Navigate to tools->Nios II software build tools for eclipse and navigate to the COMPSYS700SOPC folder
->Once in the folder goto file->import->General->Existing Projects Into Workspace and navigate to COMPSYS700SOPC/software folder
...
->Program should successfully be running and will be awaiting for switches sw[0] - sw[3] to be on, these would typically be corresponding to each core being ready and although we dont have 4 cores, we would still need to switch them all on regardless but please ensure that both jtag consoles have opened for each cores and are showing the message "waiting for switches"

->Once the messages are there flick the switches of sw[0]-sw[3] and you should see that cpu0 has initialized the matrices for A and B and also finished its computation of the half of the matrix and cpu1 should print the entirety of matrix C to show that both cpus have done their job.